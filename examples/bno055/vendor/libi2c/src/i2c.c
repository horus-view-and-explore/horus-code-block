...
char *i2c_get_device_desc(const I2CDevice *device, char *buf, size_t size)
{
    ...
    snprintf(
        buf,
        size,
        "Device address: 0x%x, tenbit: %s, internal(word) address: %d bytes, page max %d bytes, "
        "delay: %dms",
        device->addr,
        device->tenbit ? "True" : "False",
        device->iaddr_bytes,
        device->page_bytes,
        device->delay);
...
**	#iaddr	:	i2c_device internal address will read data from this address, no address set
*zero *	#buf	:	i2c data will read to here *	#len	:	how many data to read,
*lenght must less than or equal to buf size *	@return : 	success return read data length,
*failed -1
*/
